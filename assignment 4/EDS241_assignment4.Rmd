...
```{r, results='hide'}
linearHypothesis(mdl_a, "log_price=-1", white.adjust = "hc2")
```
...
\noindent Based on our linear hypothesis test, we can reject the null hypothesis that the price elasticity of demand for sardines is equal to -1 at a p-value of less than 0.001.
...
```{r}
tsls_d <- ivreg(log_volume ~ log_price | wind_m_s +
                  as.factor(country) + as.factor(year) + as.factor(month), data = df)
```
...
```{r}
mdl_d <- lm_robust(log_price ~ wind_m_s + as.factor(country) + as.factor(year) + as.factor(month), df)
```
